...
raw_file_no_filter=path+"Raw_data_with_noise.uvh5"
all_filter_type=np.array(["notch_filter_0.60","notch_filter_0.25","notch_filter_0.40"])
for filt in range (len(all_filter_type)):
    filter_type=all_filter_type[filt]

    filter_max=0
    if filter_type=="notch_filter_0.40":
        filter_max=40

    if filter_type=="notch_filter_0.25":
        filter_max=25

    if filter_type=="notch_filter_0.60":
        filter_max=60    
    print(filter_type)
    ...
    mode_array=np.array(["incomplete_no_filter_baseline_cut","incomplete_with_filter_baseline_cut","complete_with_filter","incomplete_with_filter"])
    ...
    gains_all_times_redcal=np.zeros(shape=(200,308),dtype=complex)

    cal_wedge_all=np.zeros(shape=(200,51,308),dtype=complex)
    mdl_wedge_all=np.zeros(shape=(200,51,308),dtype=complex)

    model_file=''
    raw_file=path+"Raw_filtered_data_deep_"+str(filter_max)+".uvh5"

    for step in range (len(mode_array)):
        mode=mode_array[step]
        print(mode)               
        if mode=="incomplete_with_filter_baseline_cut":                    
            model_file=path+"Model_incomplete_filtered_data_"+str(filter_max)+".uvh5"


        if mode=="complete_with_filter" :                   
            model_file=path+"Model_complete_filtered_data_"+str(filter_max)+".uvh5"
        ...
        for N_time in range (len(t)-1):
            ...
            if mode=="incomplete_with_filter_baseline_cut" or mode=="incomplete_no_filter_baseline_cut" :
                print("base line cut")
                for k in raw_data:
            ...
            gains_all_times_redcal[t[N_time]:t[N_time+1],:]=lincal_gains[(ant, 'Jee')]
            chisq_perfect_redcal[t[N_time]:t[N_time+1],:]=rc_red_chisq
            chisq_perfect[t[N_time]:t[N_time+1],:]=red_chisq
            gains_all_times[t[N_time]:t[N_time+1],:]=total_gains[(ant, 'Jee')]

        path2="/net/ike/vault-ike/ntsikelelo/Simulated_data_files/"
