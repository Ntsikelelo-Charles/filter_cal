...
for fil in range(3):
    ...
    F_model.write_data(Model_complete_filt_data,'/vault-ike/ntsikelelo/Simulated_data_files/UVH5_files/Model_complete_filtered_data_dpss_'+str(filter_max)+'.uvh5',overwrite=True)



    Model_incomplete_file = '/net/ike/vault-ike/ntsikelelo/Simulated_data_files/UVH5_files/Model_data_incomplete.uvh5'
    Model_incomplete = hc.frf.FRFilter(Model_incomplete_file)
    freqs = Model_incomplete.freqs/1e6
    times = (Model_incomplete.times-Model_incomplete .times.min()) * 24 * 3600  # seconds
    ...
    # filter the data!
    Model_incomplete_filt_data = copy.deepcopy(F_model.data)
    for k in Model_incomplete_filt_data:
        Model_incomplete_filt_data[k] = R @ Model_incomplete_filt_data[k]

    print("Model incomplete data filter with half_width = "+str(filter_half_width)+" and filter center = "+str(filter_center))    

    F_model.write_data(Model_incomplete_filt_data,'/vault-ike/ntsikelelo/Simulated_data_files/UVH5_files/Model_incomplete_filtered_data_dpss_'+str(filter_max)+'.uvh5',overwrite=True)



    raw_file = '/net/jake/home/ntsikelelo/Simulated_data_files/UVH5_files/Raw_data_with_noise.uvh5'
    ...
    F_model.write_data(raw_filt_data,'/vault-ike/ntsikelelo/Simulated_data_files/UVH5_files/Raw_filtered_data_deep_dpss_'+str(filter_max)+'.uvh5',overwrite=True)
